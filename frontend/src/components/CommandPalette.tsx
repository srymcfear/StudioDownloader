...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 p-4 bg-black/80">
      <div className="w-full max-w-2xl glass-panel rounded-2xl border border-slate-700/80 shadow-2xl overflow-hidden flex flex-col">
        {/* Search Bar Header */}
        <div className="flex items-center px-4 py-3.5 border-b border-slate-800 gap-3 bg-slate-900/60">
          ...
          <input
            ...
            autoFocus
          />
          <div className="flex items-center gap-1.5 text-[11px] text-slate-400 bg-slate-800/80 px-2 py-1 rounded-md border border-slate-700">
              ...
              return (
                <button
                  ...
                  className={`w-full p-3 rounded-xl flex items-center justify-between gap-3 text-left transition-all cursor-pointer ${
                    isSelected
                      ? 'bg-blue-600/25 border border-blue-500/50 text-white shadow-lg shadow-blue-500/10'
                      : 'bg-slate-900/40 border border-transparent text-slate-300 hover:bg-slate-800/60 hover:text-white'
                  }`}
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    {cmd.hotkey && (
                      <span className="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-[10px] font-mono text-slate-400 font-semibold">
                        {cmd.hotkey}
                      </span>
                    )}
                    {isSelected && (
        ...
        <div className="px-4 py-2.5 bg-slate-950/80 border-t border-slate-800/80 flex items-center justify-between text-[11px] text-slate-500">
          ...
          <span className="font-mono text-slate-400">FEAR STUDIO Workspace Pro</span>
        </div>
      </div>
    </div>
  );
